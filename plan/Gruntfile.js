module.exports = function ( grunt ) {
  ...
  var taskConfig = {
    ...
    /**
     * The `3` task just copies files from A to B. We use it here to copy
...
//        '<%= build_dir %>/assets/<%= pkg.name %>-<%= pkg.version %>.css': '<%= app_files.less %>'
      ...
      compile: {
        files: {
          '<%= compile_dir %>/assets/<%= pkg.name %>.css': '<%= app_files.less %>'
//        '<%= compile_dir %>/assets/<%= pkg.name %>-<%= pkg.version %>.css': '<%= app_files.less %>'
    ...
    html2js: {
      ...
      layout: {
        ...
        nonull: true,
        src: [ '<%= app_files.layout %>'],
        dest: '<%= build_dir %>/templates-layout.js'
      },
    ...
    index: {
      ...
      build: {
        ...
        src: [
          '<%= vendor_files.js %>',
          '<%= build_dir %>/src/**/*.js', //Here we take the build directories' js
          '<%= html2js.common.dest %>',
          '<%= html2js.app.dest %>',
          '<%= html2js.layout.dest %>',
          '<%= vendor_files.css %>',
...
//        '<%= vendor_files.css %>', - already concatenated, so not needed here
          '<%= build_dir %>/assets/<%= pkg.name %>.css'
//        '<%= build_dir %>/assets/<%= pkg.name %>-<%= pkg.version %>.css'
        ],
        nonull: true,
  ...
  grunt.registerTask( 'compile', [
    'less:compile', 'cssmin:compress', 'copy:compile_assets', 'ngAnnotate', 'concat:compile_js', 'uglify', 'index:compile'
  ]);
